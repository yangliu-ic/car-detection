...
def read_image_and_resize(tup):
    ...
    """
    image_path, new_WH, save_dir = tup
...
def read_flags():
    ...
    parser.add_argument("--target_csv",
                        default="labels_resized_crowdai.csv",
...
def generate_mask_pipeline(tup):
    ...
    """
    image_WH, image_path, dataframe, save_dir= tup
...
def main(FLAGS):
    ...
    data = data[(data['Label']=='Car') | (data['Label']=='Truck')].reset_index()
    data["Frame"] = data["Frame"].map(lambda x: "../UNet-in-Tensorflow-master/object-detection-crowdai/" + x)
    ...
    start = time.time()
    pool = Pool()
    pool.map_async(read_image_and_resize, [(image_path, new_WH, save_dir) for image_path in data["Frame"].unique()])
    ...
    create_clean_dir("mask_crowdai")
    logger.info("Cleaned {} directory".format("mask_crowdai"))
    logger.info("Masking begin")
    start = time.time()

    pool = Pool()
    tasks = [(new_WH, image_path, labels, "mask_crowdai")for image_path in labels["Frame"].unique()]
    pool.map_async(generate_mask_pipeline, tasks)
    pool.close()
    pool.join()
    end = time.time()
    logger.info("Masking ends. Time elapsed: {}".format(end - start))

    labels["Mask"] = labels["Frame"].map(lambda x: "mask_crowdai/" + os.path.basename(x))
