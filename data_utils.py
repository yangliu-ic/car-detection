...
def read_flags():
    ...
    parser.add_argument("--src_csv",
                        default="../UNet-in-Tensorflow-master/object-dataset/labels.csv",
...
def main(FLAGS):
    ...
    data = pd.read_csv(FLAGS.src_csv, header=None, delim_whitespace=True,
                       names=['Frame', 'xmin', 'ymin', 'xmax', 'ymax', 'ind', 'Label', 'RM'])
    data = data[(data['Label']=='car') | (data['Label']=='truck')].reset_index()
    data["Frame"] = data["Frame"].map(lambda x: "../UNet-in-Tensorflow-master/object-dataset/" + x)
